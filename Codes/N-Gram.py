...
for i in range(0, 5):
    ...
    plt.legend() 
    plt.show()
    plt.savefig("{} - N-Gram Error Analysis.png".format(classifiers[i]))
    plt.clf()
np.save("N-gram.csv", score_table)
np.savetxt("N-gram.csv", score_table, delimiter=",")
np.savetxt("N-gram - Train.csv", score_table_train, delimiter=",")